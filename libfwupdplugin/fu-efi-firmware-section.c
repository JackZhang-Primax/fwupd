...
#define FU_EFI_FIRMWARE_SECTION_OFFSET_NAME			0x04	/* only GUID defined */
			       ...
			       GError **error)
{
	...
	if (priv->type == FU_EFI_FIRMWARE_SECTION_TYPE_GUID_DEFINED) {
		...
		if (!fu_memcpy_safe ((guint8 *) &guid, sizeof(guid), 0x0,		/* dst */
				     buf, bufsz, FU_EFI_FIRMWARE_SECTION_OFFSET_NAME,	/* src */
				     sizeof(guid), error))
			return FALSE;
	...
	blob = fu_common_bytes_new_offset (fw,
					   FU_EFI_FIRMWARE_SECTION_SIZE,
					   size - FU_EFI_FIRMWARE_SECTION_SIZE,
					   error);
	if (blob == NULL)
		return FALSE;
	fu_firmware_set_bytes (firmware, blob);
	fu_firmware_set_size (firmware, g_bytes_get_size (fw));
...
fu_efi_firmware_section_write (FuFirmware *firmware, GError **error)
{
	...
	fu_byte_array_append_uint32 (buf,
				     g_bytes_get_size (blob) + FU_EFI_FIRMWARE_SECTION_SIZE,
				     G_LITTLE_ENDIAN);
	buf->data[FU_EFI_FIRMWARE_SECTION_OFFSET_TYPE] = priv->type; /* overwrite the LSB of size */
	if (priv->type == FU_EFI_FIRMWARE_SECTION_TYPE_GUID_DEFINED) {
		fwupd_guid_t guid = { 0x0 };
	...
	/* blob */
	g_byte_array_append (buf, (const guint8 *) g_bytes_get_data (blob, NULL), g_bytes_get_size (blob));
